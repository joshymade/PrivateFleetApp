...
import { updateStopTrailerNumber } from "@/lib/loads/actions";

type Variant = "page" | "panel" | "form";

/**
 * Trailer # for a stop — primary save via check button; blur / Enter as backup.
 * Keeps pickup trailer entry quick without a full form submit.
 * Parent should remount via `key` when the saved trailer changes after refresh.
 */
export function StopTrailerField({
  stopId,
  trailerNumber,
  canEdit,
  variant = "page",
  ...
  onSaved?: (trailerNumber: string | null) => void;
}) {
  const router = useRouter();
  const [value, setValue] = useState(trailerNumber ?? "");
  ...
  function save(next: string) {
    ...
    const previous = (trailerNumber ?? "").trim();
    if (nextTrimmed === previous) return;

    setError(null);
    startTransition(async () => {
      const result = await updateStopTrailerNumber(
        stopId,
        nextTrimmed || null,
      );
      if (result.error) {
        setError(result.error);
        setValue(trailerNumber ?? "");
        return;
      }
  ...
  if (!canEdit) {
    if (!trailerNumber?.trim()) return null;
    return (
      <p
        className={
          variant === "panel"
  ...
  const field = (
    <>
      ...
      <div className={isForm ? "flex items-center gap-2" : "flex min-h-11 items-center gap-2"}>
        {!isForm ? <span className={labelClass}>Trailer</span> : null}
        <input
          type="text"
          inputMode="text"
          autoComplete="off"
          value={value}
          disabled={pending}
          placeholder={isForm ? "Trailer picked up here" : "Pickup trailer #"}
          aria-label="Stop trailer number"
          onChange={(e) => {
            setJustSaved(false);
        ...
        <button
          ...
          disabled={!canSave}
          aria-label={
            pending ? "Saving trailer" : justSaved ? "Trailer saved" : "Save trailer"
          }
          title={justSaved ? "Saved" : "Save trailer"}
          onMouseDown={(e) => {
      ...
      {isForm ? (
        <span className="mt-1 block text-xs text-muted-foreground">
          Tap ✓ to save. Departing this stop updates the current trailer.
        </span>
      ) : null}
