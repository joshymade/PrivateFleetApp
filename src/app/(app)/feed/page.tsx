...
export default async function FeedPage({ searchParams }: FeedPageProps) {
  ...
  if (query) {
    weekCountsQuery = weekCountsQuery.ilike("asset_number", `%${query}%`);
  }
  ...
  if (query) {
    countQuery = countQuery.ilike("asset_number", `%${query}%`);
  }
  ...
  if (query) {
    reportsQuery = reportsQuery.ilike("asset_number", `%${query}%`);
  }
  if (selectedWeekParts) {
    const { startIso, endIso } = isoWeekRangeUtc(selectedWeekParts);
